...
const fs = require('fs');
const app = require('../index'); // Asegúrate de que index.js tenga: module.exports = app;

describe('API de usuarios', () => {
    const testUser = { id: 'test123', name: 'Test User', email: 'test@example.com' };

    afterAll(() => {
        // CORRECCIÓN 1: writeFileSync (con S mayúscula)
    ...
    it('Debe crear un nuevo usuario', async () => {
        // CORRECCIÓN 2: Primero realizamos la petición, luego evaluamos
        const res = await request(app).post('/users').send(testUser);
        expect(res.statusCode).toBe(201);
        // CORRECCIÓN 3: toMatchObject (con la letra O, no con el número 0)
    ...
    it('Debe buscar el usuario creado', async () => {
        ...
        // Si el endpoint devuelve un solo usuario, quita el Array.isArray o cámbialo según tu lógica
        expect(res.statusCode).toBe(200);
    });
});